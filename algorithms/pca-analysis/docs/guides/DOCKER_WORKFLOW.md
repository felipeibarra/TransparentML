...
```bash
# Training service
docker-compose logs -f pca-analysis

# API
docker-compose logs -f pca-api
...
docker-compose logs pca-analysis

# Reconstruir sin cache
docker-compose build --no-cache pca-analysis
```

#### "Changes not reflected"
```bash
# Reiniciar servicio específico
docker-compose restart pca-analysis
...
docker-compose exec pca-analysis python scripts/train.py
...
docker-compose run --rm pca-analysis python scripts/train.py
```

Usa `exec` para servicios que están corriendo (como pca-analysis con `tail -f /dev/null`).
